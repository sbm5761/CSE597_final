...
from models import TICModel, TransformerConfig, DeeCapModel
...
def evaluate_metrics(model, test_dataloader, tokenizer, epoch): 
    ...
    with tqdm(desc='Epoch %d - evaluation' % epoch, unit='it', total=len(test_dataloader)) as pbar:
        for idx, (tokens, _, img_features) in enumerate(test_dataloader):  
            ...
            with torch.no_grad():
                text, _ = model.beam_search(img_features, model, tokenizer, 1, beam_size=5) 
...
def main(): 
    ...
    parser.add_argument('--train_data_path', default='./data/train_mine.pkl') 
    ...
    parser.add_argument('--model_type', default='tic') #tic?
