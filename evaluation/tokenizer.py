...
class PTBTokenizer(object):
    ...
    def tokenize(cls, corpus):
        ...
        tokenized_corpus = {}
        #image_id = [k for k, v in list(corpus.items()) for _ in range(len(v[0]))]
        ids= list(corpus.keys())
        #sentences = '\n'.join([c.replace('\n', ' ') for k, v in corpus.items() for c in v[0]])
        ...
        token_lines = p_tokenizer.communicate(input=sentences.rstrip()) #removed [0]
        ...
        for k, line in zip(image_id, sentences):
            ...
            tokenized_caption = ' '.join([w for w in line.rstrip().split(' ') \
                                          if w not in cls.punctuations]) #removed space
            tokenized_corpus[k].append(tokenized_caption)

        '''
        for i, line in zip(ids, sentences):
            sent= ''.join([w for w in line[0] if w not in cls.punctuations])
